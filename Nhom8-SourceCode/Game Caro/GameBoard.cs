...
namespace Game_Caro
{
    class GameBoard
    {
        ...
        public GameBoard(Panel board, TextBox PlayerName, PictureBox Avatar)
        {
            ...
            {
                new Player("Người chơi 1", Image.FromFile(Application.StartupPath + "\\images\\X2.png"),
                                        Image.FromFile(Application.StartupPath + "\\images\\X.png")),

                new Player("Người chơi 2", Image.FromFile(Application.StartupPath + "\\images\\O2.png"),
        ...
        public void ChangePlayerName(int playerIndex)
        {
            if (playerIndex >= 0 && playerIndex < ListPlayers.Count)
            {
                ...
                ListPlayers[playerIndex].ChangeName(newName);
            }
        }
        #endregion
        ...
        public void EndGame()
        {
            ...
            string winner = ListPlayers[CurrentPlayer == 1 ? 0 : 1].Name;
        ...
        #region 1 player
        //Mảng chứa các giá trị điểm tấn công dựa trên số lượng ô liên tiếp mà máy có thể tạo ra trong một hàng, cột hoặc đường chéo.
        private long[] ArrAttackScore = new long[7] { 0, 64, 4096, 262144, 16777216, 1073741824, 68719476736 }; //Mảng tấn công theo thứ tự 0 1 2 3 4 5 6

        //Mảng lưu trữ các giá trị điểm phòng thủ dựa trên số lượng ô liên tiếp của người chơi mà máy cần chặn lại.
        private long[] ArrDefenseScore = new long[7] { 0, 8, 512, 32768, 2097152, 134217728, 8589934592 };//mảng phòng thủ

        #region Calculate attack score
        private long AttackHorizontal(int CurrRow, int CurrCol)
        {
            long TotalScore = 0; // Khởi tạo tổng điểm
            int ComCells = 0; // Số ô của máy
            int ManCells = 0; // Số ô của người chơi
            ...
            {
                if (MatrixPositions[CurrRow - Count][CurrCol].BackgroundImage == ListPlayers[0].Symbol)
                    ComCells += 1; // Tương tự như trên
                else if (MatrixPositions[CurrRow - Count][CurrCol].BackgroundImage == ListPlayers[1].Symbol)
                {
                    ManCells += 1; // Tương tự như trên
                    break;
                }
                else
                    break; // Dừng nếu ô trống
            ...
            if (ManCells == 2)
                return 0; // Nếu có 2 ô của người chơi, không tính điểm

            // Tính điểm phòng thủ và tấn công
